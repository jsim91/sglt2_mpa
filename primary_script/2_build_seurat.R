...
sink(file = file.path(here::here("output/logs"), paste0("pbmc_console_redirect_",strftime(Sys.time(),"%Y-%m-%d_%H%M%S"),".txt")), 
     type = "output")
...
soc_fil <- list.files(path = here::here("primary_dependents/souporcell"), pattern = "clusters\\.tsv$", 
                      all.files = TRUE, full.names = TRUE, recursive = TRUE, include.dirs = TRUE)
solo_fil <- list.files(path = here::here("intermediate"), pattern = "solo_scores", 
                       all.files = TRUE, full.names = TRUE, recursive = TRUE, include.dirs = TRUE)
snp_fil <- list.files(path = here::here("primary_dependents/snp_correlation"), pattern = "\\.txt$", 
                      all.files = TRUE, full.names = TRUE, recursive = TRUE, include.dirs = TRUE)
project_key <- read.csv(file = here::here("primary_dependents/MM148_key.csv"), check.names = FALSE)
...
for(i in 1:length(lanes)) {
  ...
  print(paste0("finished ",lanes[i]))
  
  seu_list[[i]] <- seu
}
dir.create(here::here("intermediate/seurat/all_cells"), showWarnings = FALSE, recursive = TRUE)
saveRDS(object = qc_metrics, file = here::here("intermediate/seurat/all_cells/mm_pbmc_lane_qc_metrics.rds"))
saveRDS(object = seu_list, file = here::here("intermediate/seurat/all_cells/mm_pbmc_lane_list.rds"))
if(F) {
  seu_list <- readRDS(file = here::here("intermediate/seurat/all_cells/mm_pbmc_lane_list.rds"))
}
...
write.csv(x = obj_meta, file = here::here("intermediate/seurat/seurat_meta.csv"), quote = FALSE, row.names = FALSE)
write.table(x = data.frame(gene = gene_names), file = here::here("intermediate/seurat/gene_names.csv"), 
