...
mydf <- readRDS('primary_dependents/pbmc2data.rds')
...
# pbmc2mod <- sccomp_estimate(mydf, formula_composition = ~ time + (1|subject_id), .sample = sample, .cell_group = cell_type, .abundance = count, bimodal_mean_variability_association = TRUE, cores = 1, verbose = FALSE, mcmc_seed=7, output_directory='pbmc2mod_output') 
# saveRDS(pbmc2mod, file='pbmc2mod.rds')

# pbmc2modc <- sccomp_estimate(mydf, formula_composition = ~ time + drug + (1|subject_id), .sample = sample, .cell_group = cell_type, .abundance = count, bimodal_mean_variability_association = TRUE, cores = 1, verbose = FALSE, mcmc_seed=7, output_directory='pbmc2modc_output') 
# saveRDS(pbmc2modc, file='pbmc2modc.rds')


### not including drug as a covariate ###

fit1 <- readRDS('primary_dependents/pbmc2mod.rds')
...
fit2 <- readRDS('primary_dependents/pbmc2modc.rds')
