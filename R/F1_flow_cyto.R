...
base_outpath <- 'D:/fluorescence/sadie/result/limited_panel_multi_res_5NOV2025'
drop_markers <- c('CD56')
...
result_outpath <- file.path(base_outpath, paste0('pass_',iteration_run))
if(!dir.exists(result_outpath)) {
  dir.create(result_outpath, recursive = TRUE)
}

if(iteration_run!=1) {
  prev_iteration_path <- file.path(base_outpath, paste0('pass_',iteration_run-1))
  if(sum(!regexpr(pattern = '\\.RData$', text = list.files(path = prev_iteration_path))==-1)!=0) {
    fcs_obj_fil <- list.files(path = prev_iteration_path, pattern = '\\.RData', full.names = TRUE)
    if(length(fcs_obj_fil)!=1) {
      stop("failed to find the expected RData file")
    } else {
      load(fcs_obj_fil)
    }
    cluster_count <- FCSimple::fcs_calculate_abundance(fcs_join_obj = fcs_obj, report_algorithm = 'leiden', 
                                                       report_as = 'count', return_abundance = TRUE)
    cluster_sums <- colSums(cluster_count)
    as_percent <- cluster_sums/sum(cluster_sums) * 100
    drop_clusters <- as.numeric(names(as_percent)[as_percent<drop_abundance])
    if(length(drop_clusters)==0) {
      stop('no clusters met the threshold for removal')
    }
    write.csv(x = data.frame(dropped_clusters = drop_clusters), 
              file = file.path(prev_iteration_path, paste0('drop_clusters_for_pass_',iteration_run,'.csv')), row.names = F)
    keep_index <- !fcs_obj$leiden$clusters %in% drop_clusters
    fcs_obj <- list(data = fcs_obj$data[keep_index,], 
                    source = fcs_obj$source[keep_index], 
                    pid = fcs_obj$pid[keep_index], 
                    run_date = fcs_obj$run_date[keep_index])
    iteration_flag <- 1
  } else {
    iteration_flag <- 0
  }
