# Local copy of core DGE helpers from jsim91/seutools (seurat_in.R).
...
seu_mast_gsea <- function(mast_dge_result, seu_mast_sets,
  ...
  if (sum(duplicated(mast_dge_result$raw_res$primerid)) != 0) {
    stop("mast_dge_result$raw_res$primerid must not contain duplicates")
  }
  ...
  if (is.null(mast_zlmfit) || is.null(sca)) {
    stop("mast_dge_result must contain 'zlmfit' and 'sca' elements.")
  }

  if (verbose) {
    if (num_boots <= 50) {
      warning(paste0("'num_boots' = ", num_boots, ". MAST authors recommend >50 for reported values."))
    }
    print("Looking for *_modules_gsea.gmt files in local search directories.")
  }

  gmt_dirs <- unique(c(
    gmt_search_dir,
    file.path(getwd(), "gene_set"),
    getwd()
  ))
  gmt_dirs <- gmt_dirs[!is.na(gmt_dirs) & nzchar(gmt_dirs)]
  gmt_dirs <- gmt_dirs[file.exists(gmt_dirs)]
  if (length(gmt_dirs) == 0) {
    stop("No valid GMT search directories found. Provide 'gmt_search_dir' or place *.gmt files under ./gene_set.")
  }

  list_module_files <- unique(unlist(lapply(gmt_dirs, function(d) {
    list.files(path = d, pattern = "modules_gsea\\\\.gmt$", full.names = TRUE)
  })))
  if (length(list_module_files) == 0) {
    stop("No '*_modules_gsea.gmt' files found in local search directories.")
  }

  db_pattern <- paste0("(", paste0(seu_mast_sets, collapse = "|"), ")")
  module_files <- list_module_files[grep(pattern = db_pattern, x = basename(list_module_files))]
  if (length(module_files) == 0) {
    stop("No requested gene-set DB GMT files found. Check 'seu_mast_sets' names and local GMT files.")
  }

  bootVcov1_fn <- get("bootVcov1", asNamespace("MAST"))
  pbootVcov1_fn <- get("pbootVcov1", asNamespace("MAST"))
  gseaAfterBoot_fn <- get("gseaAfterBoot", asNamespace("MAST"))
  CoefficientHypothesis_fn <- get("CoefficientHypothesis", asNamespace("MAST"))

  if (round(nthread) <= 1) {
    boots <- bootVcov1_fn(zlmfit = mast_zlmfit, R = num_boots)
  } else {
    nthread <- round(nthread)
    makecl <- parallel::makePSOCKcluster(nthread)
    on.exit(parallel::stopCluster(makecl), add = TRUE)
    boots <- pbootVcov1_fn(zlmfit = mast_zlmfit, R = num_boots, cl = makecl)
  }

  gsea_result <- vector("list", length = length(module_files))
  for (i in seq_along(module_files)) {
    module_file <- module_files[i]
    target_module <- stringr::str_extract(string = basename(module_file), pattern = db_pattern)
    names(gsea_result)[i] <- target_module

    gene_set <- GSEABase::getGmt(module_file)
    gene_ids <- GSEABase::geneIds(gene_set)
    sets_indices <- limma::ids2indices(gene_ids, S4Vectors::mcols(sca)$primerid)
    ...
    if (length(keep_gs_indices) == 0) {
      stop("No gene sets to test with current 'gs_min', 'gs_max', and 'gs_regex'.")
    }
    sets_indices <- sets_indices[keep_gs_indices]

    geneset_meta <- vector("list", length = length(sets_indices))
    names(geneset_meta) <- names(sets_indices)
    for (j in seq_along(sets_indices)) {
      gs_name <- names(sets_indices)[j]
      geneset_res <- mast_dge_result$raw_res[gene_ids[[gs_name]], ]
      geneset_res$primerid <- gene_ids[[gs_name]]
      geneset_res$geneset <- gs_name
      row.names(geneset_res) <- NULL
      geneset_meta[[j]] <- geneset_res
    }

    gsea <- gseaAfterBoot_fn(mast_zlmfit, boots, sets_indices, CoefficientHypothesis_fn("categoryGroup2"))
    gsea_table <- summary(gsea, testType = "normal")

    gsea_table_top <- gsea_table[gsea_table$combined_Z > 0, ]
    gsea_table_top <- gsea_table_top[order(gsea_table_top$combined_Z, decreasing = TRUE), ]
    if (nrow(gsea_table_top) > prepare_plot_n) {
      gsea_table_top <- gsea_table_top[1:prepare_plot_n, ]
    }

    gsea_table_bottom <- gsea_table[gsea_table$combined_Z < 0, ]
    gsea_table_bottom <- gsea_table_bottom[order(gsea_table_bottom$combined_Z, decreasing = FALSE), ]
    if (nrow(gsea_table_bottom) > prepare_plot_n) {
      gsea_table_bottom <- gsea_table_bottom[1:prepare_plot_n, ]
    }

    gsea_top_bottom_table <- rbind(
      gsea_table_top,
      gsea_table_bottom[order(gsea_table_bottom$combined_Z, decreasing = TRUE), ]
    )
    gseaTable_plot <- data.table::melt(gsea_top_bottom_table[, c("set", "disc_Z", "cont_Z", "combined_Z")], id.vars = "set")
    gseaTable_report <- as.data.frame(gsea_table)
    gseaTable_report$positive_score_group <- positive_group

    gsea_result[[i]] <- list(
      result = gseaTable_report,
